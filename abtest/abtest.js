// TODO(DEVELOPER): Change the values below using values from the initialization snippet: Firebase Console > Overview > Add Firebase to your abtest app.
// Initialize Firebase
var config = {
...
var data;
//get data from database for topic: abtest
...
function displayData(type) {
	for (var x in data[type]) {
		...
		comment_button.addEventListener('click', function(e) {
			...
			if (data.value) {
				var pushComment = firebase.database().ref("abtest/" + type + "/" + id).child('comment').push();
				pushComment.set({
					data: data.value,
		...
		useful_button.addEventListener('click', function(e) {
			...
			num_useful++
			firebase.database().ref("abtest/" + type + "/" + id).update({
				useful: num_useful
			}).then(() => {
